...
def generate_blueprint(context: dict, maturity: dict, recommendations: list, compliance: dict) -> dict:
    ...
    blueprint = {
        ...
        "executive_summary": {
            "current_level": f"Level {maturity['current_level']} - {maturity['current_name']}",
            "target_level": f"Level {maturity['target_level']} - {maturity['target_name']}",
            "timeline": f"{maturity['timeline_months']} months",
            "total_phases": maturity["total_phases"],
            "compliance_status": compliance["overall_status"],
            "recommended_monthly_cost": f"${cost_summary['recommended_path']['monthly']}/month",
        },
